...
# EV charging: Tesla's energy API doesn't report vehicle_w separately,
# so we pass through whatever Tesla gives us (typically 0).
# No heuristic estimation — all home load is reported as "Home".
...
def _compute_sankey_flows(rows) -> dict:
    """Compute Sankey flow allocations from raw 5-minute intervals.

    Each interval has a clear single direction for grid and battery,
    eliminating the false cross-flows caused by hourly averaging.
    """
    flows = {
        "solar_to_home": 0.0,
    ...
    for row in rows:
        # Convert watts to kWh for this 5-min interval
    ...
    return {k: round(v, 3) for k, v in flows.items()}
...
async def sankey(
    ...
    """Sankey flow allocations computed at 5-minute interval granularity."""
    pool = request.app.state.pool
    ...
    if start and end:
        ...
        range_start = range_end = datetime.now(LOCAL_TZ).date()

    dt_start = datetime.combine(range_start, time.min, tzinfo=LOCAL_TZ)
    dt_end = datetime.combine(range_end + timedelta(days=1), time.min, tzinfo=LOCAL_TZ)
    ...
    flows = _compute_sankey_flows(rows)
